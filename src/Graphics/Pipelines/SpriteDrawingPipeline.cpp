#include "SpriteDrawingPipeline.h"
    ...
    std::size_t maxSprites)
{
    ...
    const auto layouts = std::array{bindlessDescriptorSetLayout};
    ...
    const std::vector<SpriteDrawCommand>& spriteDrawCommands)
{
    ...
    const auto& commandBuffer = getCurrentFrameData(frameIndex).spriteDrawCommandBuffer;
